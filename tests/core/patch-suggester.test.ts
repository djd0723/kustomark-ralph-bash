...
Upgrade to 2.0.0 today`;
  ...
  describe("line insertion detection", () => {
    ...
    test("describePatch returns human-readable description for insert-before-line", () => {
      // insert-before-line is not generated by suggestPatches for file-start insertions
      // (prepend-to-file is preferred). Test the describePatch function directly via scorePatches
      // using a mid-file scenario where a custom insert-before-line patch is scored.
      const source = "Line A.\nLine B.";
      const target = "Line A.\nInserted.\nLine B.";

      const patches = suggestPatches(source, target);
      const scored = scorePatches(patches, source, target);
      // insert-after-line is generated for mid-file insertions
      const insertAfterScored = scored.find((s) => s.patch.op === "insert-after-line");
      expect(insertAfterScored).toBeDefined();
      expect(insertAfterScored?.description).toContain("Insert content after line");
    });
  });
