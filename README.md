...
## File-Level Content Operations

Add content to the very beginning or end of every matched file. Useful for auto-generation notices, headers, footers, and watermarks.

### `prepend-to-file` - Add Content to File Start

Insert content before the very first character of the file.

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `op` | string | ✓ | `"prepend-to-file"` |
| `content` | string | ✓ | Content to insert at the beginning of the file |
| `include` | string \| string[] | | Glob pattern(s) to target specific files |
| `exclude` | string \| string[] | | Glob pattern(s) to exclude specific files |
| `onNoMatch` | string | | Override default `onNoMatch` behavior (this op always succeeds) |

```yaml
# Add an auto-generation notice before all files
- op: prepend-to-file
  content: |
    <!-- This file is auto-generated by kustomark. Do not edit directly. -->
...
**Notes:**
- `count` is always 1 — the operation unconditionally prepends
- Content is inserted verbatim with no separator added automatically; include a trailing newline in `content` if needed
- Use `include`/`exclude` to limit which files are affected
- Symmetrical with `append-to-file`

### `append-to-file` - Add Content to File End

Append content after the very last character of the file.

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `op` | string | ✓ | `"append-to-file"` |
| `content` | string | ✓ | Content to insert at the end of the file |
| `include` | string \| string[] | | Glob pattern(s) to target specific files |
| `exclude` | string \| string[] | | Glob pattern(s) to exclude specific files |
| `onNoMatch` | string | | Override default `onNoMatch` behavior (this op always succeeds) |

```yaml
# Add a footer to all markdown files
- op: append-to-file
  content: |

    ---

    *Generated by kustomark — [source](https://github.com/org/repo)*

# Add a machine-readable tag at the end
- op: append-to-file
  content: |

    <!-- kustomark:team=engineering -->
  include: "docs/**/*.md"
```

**Notes:**
- `count` is always 1 — the operation unconditionally appends
