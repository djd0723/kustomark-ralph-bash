...
/**
 * Apply a prepend-to-file patch operation to add content at the very beginning of a file.
 *
 * Inserts new content before the first character of the file. If the file has frontmatter,
 * the content is inserted before the frontmatter block.
 *
 * @param content - The file content to patch
 * @param prependContent - The content to insert at the beginning
 * @returns Object with patched content and count (always 1)
 *
 * @example
 * ```typescript
 * const result = applyPrependToFile(
 *   '# My Doc\n\nSome text.',
 *   '<!-- AUTO-GENERATED -->\n\n'
 * );
 ...
 * ```
 */
export function applyPrependToFile(
  content: string,
  prependContent: string,
): { content: string; count: number } {
  return { content: prependContent + content, count: 1 };
}

/**
 * Apply an append-to-file patch operation to add content at the very end of a file.
 *
 * Appends new content after the last character of the file. Ensures a newline separator
 * if the file does not already end with a newline.
 *
 * @param content - The file content to patch
 * @param appendContent - The content to append at the end
 * @returns Object with patched content and count (always 1)
 *
 * @example
 * ```typescript
 * const result = applyAppendToFile(
 *   '# My Doc\n\nSome text.',
 ...
 * ```
 */
export function applyAppendToFile(
  content: string,
  appendContent: string,
): { content: string; count: number } {
  return { content: content + appendContent, count: 1 };
}
...
export async function applySinglePatch(
  ...
  switch (patch.op) {
    ...
    case "prepend-to-file":
      result = applyPrependToFile(content, patch.content);
      break;

    case "append-to-file":
      result = applyAppendToFile(content, patch.content);
      break;
