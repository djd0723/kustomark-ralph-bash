...
**2026-04-10 (Profiling & Benchmarking Test Coverage - COMPLETE!):**

* ✅ **`memory-profiler.ts` unit tests**: 55 tests across 11 describe groups in `tests/core/memory-profiler.test.ts` covering construction, lifecycle (start/stop), heap snapshots, GC recording, sampling intervals, all three leak detection algorithms (growing-memory, gc-ineffective, growing-allocations), `generateMemoryReport()` (JSON + text), multiple sessions, and edge cases
* ✅ **`profile` CLI tests**: 24 tests in `tests/cli/profile.test.ts` covering subcommand routing, all four scenarios (small/medium/large/custom), `--format json` shape validation, `--output` file writing, `--sampling-interval` passthrough, and invalid argument error paths
* ✅ **`benchmark` CLI tests**: 15 tests in `tests/cli/benchmark.test.ts` covering `benchmark run` (text/json/markdown output), `--operations` filtering, `benchmark list`, `--save-baseline` / `--baseline` comparison, and unknown-subcommand error handling
* ✅ **`kustomark profile` documented in README.md**: New `kustomark profile memory` section inserted between the benchmark and web sections — includes usage examples for every scenario and option, an options table, scenario breakdown, and JSON/text output examples
* ✅ **4,754 tests passing**: 94 new tests added (4,660 → 4,754), 0 failures, `bun check` clean
